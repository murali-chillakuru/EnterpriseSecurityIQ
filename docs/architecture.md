...
> High-level pipeline design, data flow, and component overview. No internal file paths or code internals.

## Pipeline Overview

PostureIQ operates as a four-stage pipeline:

```
Collectors → Normalization → Evaluation → Reports
```

1. **Collectors** call Azure ARM, data-plane, MS Graph, and Power Platform APIs
2. **Normalization** converts raw responses into unified evidence with deterministic IDs
3. **Evaluation** maps evidence to framework controls and runs domain-specific checks
4. **Reports** generate professional output in multiple formats
...
│ Entra (async)    │    │              │    │ 11 frameworks    │    │ 29 modules     │    │          │
...
PostureIQ uses 68 async collectors (50 Azure + 18 Entra) that run concurrently with:

- **Retry logic** — exponential backoff for transient failures
- **Pagination** — automatic next-link following for large result sets
- **Checkpoint resume** — skip already-collected evidence types on restart
- **Access-denied handling** — graceful degradation when permissions are insufficient
- **Shared inventory cache** — resource lists are collected once and shared across collectors

### Azure Collectors (50)

Cover 29 resource categories including: Virtual Machines, Storage Accounts, Key Vaults, SQL Databases, AKS Clusters, App Services, Functions, Cosmos DB, Networking (NSGs, Firewalls, Route Tables, Front Door), Policy, Defender, and more.

### Entra ID Collectors (18)

Cover: Users, Groups, App Registrations, Service Principals, Conditional Access Policies, Directory Roles, PIM Assignments, Named Locations, Auth Methods, Sign-in Logs, Risk Detections, Risky Users, OAuth2 Grants, Access Reviews, and more.

## Normalization

Raw API responses are converted into unified evidence dictionaries:

- **Deterministic IDs** — UUID5 generated from evidence type + resource identifier (same input always produces same ID)
- **Unified key casing** — PascalCase throughout
- **218 distinct evidence types** cataloged

## Evaluation Engine

The evaluation engine processes evidence through 10 security domains:

| Domain | Check Functions | Focus |
|--------|----------------|-------|
| Access Control | 8 | RBAC separation, least privilege, conditional access |
| Identity & Auth | 22 | MFA, credentials, user lifecycle, risky users |
| Data Protection | 22 | Encryption, Key Vault, VM/SQL/AKS hardening |
| Logging & Monitoring | 13 | Diagnostics, threat detection, sign-in monitoring |
| Network Security | 16 | NSG rules, firewalls, storage firewalls, WAF |
| Governance & Risk | 21 | Policy compliance, Defender, PIM, access reviews |
| Incident Response | 6 | Security contacts, alerting, Sentinel |
| Change Management | 4 | Change control, resource locks |
| Business Continuity | 4 | Backup, geo-redundancy, availability |
| Asset Management | 4 | Inventory, classification, tagging |

Each check function produces a finding with:
- **Status** — Compliant, Non-Compliant, Not Applicable, or Error
- **Severity** — Critical, High, Medium, Low, or Informational
- **Evidence** — Link to the collected evidence that triggered the finding
- **Remediation** — Recommended fix actions

## Framework Mapping

Findings are mapped to 11 compliance frameworks (525 total controls). Each framework defines which evaluation domains and controls apply. Multiple frameworks can run in a single assessment.

## Report Generation

29 report modules produce 8 output formats:

| Format | Description |
|--------|-------------|
| **HTML** | Interactive report with sidebar navigation, score rings, domain cards, remediation roadmap |
| **JSON** | Machine-readable structured output with full evidence and metadata |
| **Markdown** | Text-based report for documentation and review workflows |
| **Excel** | Spreadsheet with per-finding rows, filterable columns |
| **OSCAL** | NIST Open Security Controls Assessment Language (machine interchange) |
| **SARIF** | Static Analysis Results Interchange Format (CI/CD integration) |
| **PDF** | Print-ready report with Playwright-based rendering |
| **Webhook** | JSON payload pushed to Sentinel, Splunk, or custom endpoints |

Every report includes a **SHA-256 integrity hash** for tamper-evidence verification.

## Agent Architecture

When running as a Foundry agent, PostureIQ exposes 14 tools via the Microsoft Agent Framework SDK. The agent uses the `responses` protocol and runs on port 8088.

The agent layer wraps the same pipeline used by CLI scripts — no separate logic paths.

## Concurrency Model

- Collectors run as async tasks with configurable concurrency limits
- Per-collector semaphores prevent API throttling
- The orchestrator coordinates the full pipeline: authentication → collection → evaluation → reporting
- All stages support partial completion — if a collector fails, the pipeline continues with available evidence
...
├── webapp/                              # 8 self-contained HTML SPAs with MSAL auth + SSE streaming
│   ├── index.html                       # Portal — card-grid launcher
│   ├── SecurityComplianceAssessment.html # Security Compliance Assessment
│   ├── RiskAnalysis.html                # Risk Assessment
│   ├── DataSecurity.html                # Data Security Assessment
│   ├── RBACReport.html                  # RBAC Assessment
│   ├── CopilotReadiness.html            # Copilot Readiness Assessment
│   ├── AIAgentSecurity.html             # AI Agent Security Assessment
│   └── CloudExplorer.html               # Cloud Explorer — live ARG/Graph queries
├── config/
