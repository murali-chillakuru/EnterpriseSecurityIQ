...
async def run_ai_agent_security_assessment(
    ...
    # Log absent evidence categories so operators see what couldn't be assessed
    _EXPECTED_EVIDENCE_TYPES = {
        "copilot-studio-bot", "foundry-config-summary",
        "azure-openai-deployment", "azure-ai-workspace",
        "foundry-project", "foundry-agent-application",
        "azure-ai-connection", "foundry-capability-host",
        "azure-ai-workspace-diagnostics", "azure-ai-prompt-shield",
        "entra-service-principal", "entra-conditional-access",
    }
    present = set(evidence_index.keys())
    absent = _EXPECTED_EVIDENCE_TYPES - present
    if absent:
        log.warning("Evidence types absent from index (evaluators will report unable-to-assess): %s",
                     ", ".join(sorted(absent)))
