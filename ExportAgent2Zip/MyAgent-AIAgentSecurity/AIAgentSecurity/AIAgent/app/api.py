...
from app.agent import SYSTEM_PROMPT, TOOLS, get_session_context_summary, get_completed_assessment_tools, get_last_tenant_search_meta, clear_session, _request_conversation_id
from app.auth import ComplianceCredentials, UserTokenCredential, _request_creds
from app.query_frameworks.advisor import get_security_context
...
# Allowed CORS origins — comma-separated.  Empty string → same-origin only.
...
TOOL_SCHEMAS = [
    ...
    {"type": "function", "function": {"name": "run_attack_path_detection", "description": "Detect cross-domain attack paths in the tenant: consent abuse, privilege escalation, lateral movement, credential chains, exposed high-value resources, CA bypass, PIM escalation, network pivot. Maps to MITRE ATT&CK with per-step playbooks and compliance framework relevance.", "parameters": {"type": "object", "properties": {"min_severity": {"type": "string", "description": "Minimum severity to include in the report (informational, low, medium, high, critical). Default 'informational' returns all detected paths.", "enum": ["informational", "low", "medium", "high", "critical"]}, "subscriptions": {"type": "string", "description": "Optional comma-separated subscription IDs"}}}}},
    ...
    {"type": "function", "function": {"name": "generate_custom_report", "description": "Generate a focused custom report (HTML / PDF / Excel) from existing session assessment data on a specific topic. Does NOT re-run assessments.", "parameters": {"type": "object", "properties": {"topic": {"type": "string", "description": "Natural-language description of what the report should cover, e.g. 'critical storage findings', 'all high-severity issues'"}, "format": {"type": "string", "description": "Output format: 'html', 'pdf', 'excel', or 'all'", "enum": ["html", "pdf", "excel", "all"]}, "severity": {"type": "string", "description": "Optional comma-separated severity filter: 'critical', 'high', 'medium', 'low'"}, "category": {"type": "string", "description": "Optional comma-separated category filter (e.g. 'storage,database')"}}, "required": ["topic"]}}},
]
...
PAGE_ALLOWED_TOOLS: dict[str, set[str]] = {
    "SecurityComplianceAssessment": {"run_postureiq_assessment", "generate_rbac_report", "analyze_risk", "search_exposure", "generate_custom_report"},
    "DataSecurity":     {"assess_data_security", "search_exposure"},
    "AttackPath":       {"run_attack_path_detection"},
    ...
    "CloudExplorer":    {"search_exposure"},  # cloud explorer + exposure scan + permissions
}
ALL_ASSESSMENT_TOOLS = {
    ...
    "assess_ai_agent_security", "run_attack_path_detection",
}
...
async def lifespan(app: FastAPI):
    ...
    if not OPENAI_ENDPOINT:
        raise RuntimeError(
            "AZURE_OPENAI_ENDPOINT is not set. The agent requires an Azure OpenAI "
            "endpoint to function. Set the environment variable and restart."
        )
...
app = FastAPI(
    title="PostureIQ API", version="2.0.0", lifespan=lifespan
)


# ── Lightweight rate limiter (per-IP, in-process) ───────────────
...
_RATE_MAX_CHAT = 20        # max /chat requests per window per IP
_RATE_MAX_ASSESS = 5       # max /assessments requests per window per IP
...
class _RateLimitMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: _StarletteRequest, call_next):
        ...
        if path in ("/chat", "/assessments"):
            limit = _RATE_MAX_CHAT if path == "/chat" else _RATE_MAX_ASSESS
...
# ── CORS (locked to specific origins; same-origin requests always pass) ──
_DEFAULT_ORIGINS = [
    "http://localhost:8088",
    "http://localhost:8090",
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=ALLOWED_ORIGINS or _DEFAULT_ORIGINS,
    allow_credentials=True,
    allow_methods=["GET", "POST", "OPTIONS"],
    allow_headers=["Content-Type", "Authorization"],
)

# ── CSP frame-ancestors (allow Teams to embed the webapp in an iframe) ──
_FRAME_ANCESTORS = "frame-ancestors 'self' https://teams.microsoft.com https://*.teams.microsoft.com https://teams.cloud.microsoft https://*.cloud.microsoft https://*.office.com https://*.microsoft365.com https://*.microsoft.com"
...
class ChatResponse(BaseModel):
    ...
    tenant_search_meta: dict[str, Any] | None = None


# ── Allowed values for assessment input validation ──────────────
_ALLOWED_FRAMEWORKS = {
    "FedRAMP", "CIS", "ISO-27001", "NIST-800-53", "PCI-DSS",
    "MCSB", "HIPAA", "SOC2", "GDPR", "NIST-CSF", "CSA-CCM",
}
_ALLOWED_OUTPUT_FORMATS = {"json", "html", "both"}


def _validate_webhook_url(url: str | None) -> str | None:
    """Reject webhook URLs that target internal/private networks (SSRF)."""
    if not url:
        return url
    ...
    try:
        ...
        if addr.is_private or addr.is_loopback or addr.is_link_local or addr.is_reserved:
            raise ValueError("webhook_url must not target private/internal addresses")
...
async def _run_and_notify(task_id: str, req: AssessmentRequest) -> None:
    ...
    try:
        ...
        if req.webhook_url:
            try:
                validated_url = _validate_webhook_url(req.webhook_url)
...
async def chat(req: ChatRequest):
    """Chat with the PostureIQ agent (SSE stream with keepalive).
    ...
    async def _sse_generator():
        ...
        if req.mode == "tenant_search":
            ...
            try:
                ...
                tenant_search_meta = await get_last_tenant_search_meta()
            except Exception as exc:
            ...
            # Enrich with security context ONLY when user EXPLICITLY asks for insights
            # Use multi-word phrases to avoid false positives from normal CE queries
            # like "show policy compliance status" or "show Defender security score"
            _sec_phrases = {
                "security insight", "security context", "security annotation",
                "compliance framework", "compliance mapping", "framework mapping",
                "risk score", "risk indicator", "risk analysis for",
                "data security context", "rbac risk", "copilot readiness",
                "ai security context", "owasp for", "nist for",
            }
            _user_q = req.message.lower()
            _has_results = tenant_search_meta and tenant_search_meta.get("has_results")
            _wants_security = _has_results and any(ph in _user_q for ph in _sec_phrases)
            security_context = ""
            if _wants_security and tenant_search_meta.get("result_family"):
                security_context = get_security_context(tenant_search_meta["result_family"])
            ...
            messages.append({"role": "system", "content": (
                ...
                "3. If the data is empty and the source is 'none', say: 'Could not match your query to a known resource type. "
                "Try asking about specific resources like VMs, storage accounts, NSGs, web apps, SQL servers, Key Vaults, AKS clusters, or Entra objects like users, groups, apps.' "
                "4. If the data is empty but a query WAS matched (source is 'arg', 'entra', or 'multi'), say: "
                "'No [resource type] found in your tenant.' — be specific about what was searched for based on the query_used field. "
                "5. If there's an error, explain the error honestly. "
                "6. Format results using markdown tables for tabular data, or structured lists for other data. "
                "7. NEVER say 'I don't have access' or 'I can't query your tenant' — the tool already did. "
                "8. Do NOT offer generic security advice. Only discuss what the data shows. "
                "9. Include counts (e.g., 'Found 42 virtual machines across 3 subscriptions'). "
                "10. If the result set appears truncated or capped (e.g., exactly 30 items), warn the user: "
                "'Note: Results may be truncated. Try a more specific query or filter by subscription/region to see all resources.'"
                + ("\n11. SECURITY ANNOTATIONS: The user explicitly asked for security context. "
                   "After presenting the data, add a '🔐 Security Insights' section. "
                   "Format ALL security insights as **markdown tables** for easy reading: "
                   "a) Compliance table: columns = Framework | Controls | Relevance. "
                   "b) Risk table: columns = Category | Score | Top Risks. "
                   "c) Recommendations table: columns = Area | Priority | Action. "
                   "Use the security context provided below. Do NOT use bullet lists — tables only." if security_context else "")
            )})
            ...
            # Inject security context for the LLM to use in annotations
            if security_context:
                messages.append({"role": "system", "content": security_context})
            ...
            yield f"data: {json.dumps({'response': choice.message.content or '', 'tools_used': tools_used, 'tenant_search_meta': tenant_search_meta})}\n\n"
            return
        ...
        try:
            for _ in range(max_rounds):
                ...
                for tc in choice.message.tool_calls:
                    ...
                    # Override scope for Data Security when SPA sent explicit category list
                    if tc.function.name == "assess_data_security" and req.frameworks:
                        args["scope"] = ",".join(req.frameworks)
                        log.info("Overriding LLM scope with explicit category list: %s", req.frameworks)
...
async def start_assessment(req: AssessmentRequest, bg: BackgroundTasks) -> AssessmentResponse:
    ...
    # Validate frameworks
    bad_fw = [f for f in req.frameworks if f not in _ALLOWED_FRAMEWORKS]
    if bad_fw:
        raise HTTPException(status_code=400, detail=f"Unknown frameworks: {bad_fw}. Allowed: {sorted(_ALLOWED_FRAMEWORKS)}")
    # Validate output formats
    bad_fmt = [f for f in req.output_formats if f not in _ALLOWED_OUTPUT_FORMATS]
    if bad_fmt:
        raise HTTPException(status_code=400, detail=f"Unknown output formats: {bad_fmt}. Allowed: {sorted(_ALLOWED_OUTPUT_FORMATS)}")
...
@app.get("/teams-init.js")
async def serve_teams_init_js():
    """Serve Teams SDK initialisation helper."""
    js = _WEBAPP_DIR / "teams-init.js"
    if js.exists():
        return FileResponse(js, media_type="application/javascript")
    return Response(status_code=404, content="teams-init.js not found")
