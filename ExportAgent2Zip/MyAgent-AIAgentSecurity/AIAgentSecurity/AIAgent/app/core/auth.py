...
class UserTokenCredential(AsyncTokenCredential):
    ...
    def __init__(self, graph_token: str, arm_token: str):
        ...
        # Derive real expiry from the JWT 'exp' claim when possible;
        # fall back to a 1-hour estimate if decoding fails.
        self._graph_expires = self._extract_exp(graph_token)
        self._arm_expires = self._extract_exp(arm_token)
        # Advisory scope check — warns if expected scopes appear absent.
        self._warn_on_missing_scopes(graph_token, "graph")
        self._warn_on_missing_scopes(arm_token, "arm")

    @staticmethod
    def _extract_exp(token: str) -> int:
        """Best-effort extraction of the 'exp' claim from a JWT.

        Returns the epoch timestamp or now+3600 as fallback.
        Note: this decodes without signature verification — the real
        validation happens when Azure ARM/Graph APIs consume the token.
        """
        try:
    ...
    @staticmethod
    def _warn_on_missing_scopes(token: str, label: str) -> None:
        """Best-effort check of JWT 'scp' (delegated) or 'roles' (app) claim.

        Logs a warning when commonly-needed scopes appear to be absent.
        This is advisory only — the authoritative validation is performed
        by Azure ARM / Graph when the token is actually used.
        """
        _EXPECTED_GRAPH_SCOPES = {
            "directory.read.all", "policy.read.all",
            "securityevents.read.all", "identityriskyuser.read.all",
        }
        _EXPECTED_ARM_SCOPES = {"user_impersonation"}
        try:
            import base64, json as _json
            payload = token.split(".")[1]
            payload += "=" * (-len(payload) % 4)
            claims = _json.loads(base64.urlsafe_b64decode(payload))
            # Delegated tokens use 'scp'; app tokens use 'roles'
            granted_raw = claims.get("scp", "") or " ".join(claims.get("roles", []))
            granted = {s.lower() for s in granted_raw.split() if s}
            if not granted:
                return  # no scope info in token — nothing to check
            expected = _EXPECTED_GRAPH_SCOPES if label == "graph" else _EXPECTED_ARM_SCOPES
            missing = expected - granted
            if missing:
                log.warning(
                    "User %s token may be missing scopes: %s  (granted: %s). "
                    "Some collectors could fail with 403.",
                    label, ", ".join(sorted(missing)), ", ".join(sorted(granted)),
                )
        except Exception:
            pass  # decoding failed — skip advisory check

    def _token_for(self, token: str, expires: int) -> AccessToken:
        remaining = expires - int(time.time())
        if remaining < 120:
            log.warning(
                "User token expires in %d seconds — API calls may fail. "
                "The user should refresh the browser page to acquire a new token.",
                remaining,
            )
    ...
    async def get_token(self, *scopes: str, **kwargs) -> AccessToken:
        ...
        if "graph.microsoft.com" in scope_str:
            return self._token_for(self._graph_token, self._graph_expires)
        if "management.azure.com" in scope_str or "management.core.windows.net" in scope_str:
            return self._token_for(self._arm_token, self._arm_expires)
...
class ComplianceCredentials:
    ...
    async def preflight_check(self) -> dict:
        ...
        result: dict = {
            ...
            "warnings": [], "errors": [],
            "rbac_assignments": [],   # Azure RBAC role assignments for the signed-in user
        }
        ...
        # 5. Azure RBAC role assignments — enumerate what roles the user has
        try:
            from azure.mgmt.authorization.aio import AuthorizationManagementClient

            # Determine the signed-in user's object ID for filtering
            user_object_id = None
            try:
                me_obj = await graph.me.get()
                user_object_id = getattr(me_obj, "id", None)
            except Exception:
                pass  # Service principal or no /me — skip filtering, show all

            subs = await self.list_subscriptions()
            rbac_list: list[dict] = []
            role_def_cache: dict[str, str] = {}

            for sub_info in subs[:10]:  # Cap at 10 subs to avoid timeout
                sub_id = sub_info["subscription_id"]
                sub_name = sub_info["display_name"]
                try:
                    auth_client = AuthorizationManagementClient(self.credential, sub_id)
                    try:
                        scope = f"/subscriptions/{sub_id}"
                        # If we have user OID, filter; otherwise list all at scope
                        if user_object_id:
                            filter_str = f"principalId eq '{user_object_id}'"
                            assignments_iter = auth_client.role_assignments.list_for_scope(
                                scope, filter=filter_str
                            )
                        else:
                            assignments_iter = auth_client.role_assignments.list_for_scope(
                        ...
                        count = 0
                        async for ra in assignments_iter:
                            if count >= 50:  # Safety cap per subscription
                                break
                            # Resolve role definition name
                            rd_id = ra.role_definition_id or ""
                            role_name = role_def_cache.get(rd_id)
                            if role_name is None and rd_id:
                                try:
                                    rd = await auth_client.role_definitions.get_by_id(rd_id)
                                    role_name = rd.role_name or rd.display_name or "Unknown"
                                except Exception:
                                    role_name = rd_id.rsplit("/", 1)[-1][:8] + "…"
                                role_def_cache[rd_id] = role_name

                            # Determine scope level
                            ra_scope = ra.scope or scope
                            if "/resourceGroups/" in ra_scope:
                                if "/providers/" in ra_scope:
                                    scope_label = "Resource"
                                else:
                                    scope_label = "Resource Group"
                            elif "/subscriptions/" in ra_scope:
                                scope_label = "Subscription"
                            elif "/managementGroups/" in ra_scope:
                                scope_label = "Management Group"
                            else:
                                scope_label = "Other"
                    ...
                    err_str = str(exc)
                    if "403" in err_str or "AuthorizationFailed" in err_str:
                        result["warnings"].append(
                            f"Cannot read role assignments on '{sub_name}' — need Reader or "
                            "Microsoft.Authorization/roleAssignments/read permission."
                        )
                    else:
                        log.debug("[preflight] RBAC enum for %s failed: %s", sub_name, exc)

            result["rbac_assignments"] = rbac_list
            if not rbac_list and subs:
                result["warnings"].append(
                    "No Azure RBAC role assignments found for your identity. "
                    "You may lack Reader access or the filter returned no results."
                )
        except ImportError:
            result["warnings"].append(
                "azure-mgmt-authorization not installed — cannot enumerate RBAC role assignments."
            )
        except Exception as exc:
            result["warnings"].append(f"RBAC role assignment enumeration failed: {exc}")
