...
async def paginate_graph(request_builder, top: int = 999, max_retries: int = 3) -> list[Any]:
    ...
    try:
        ...
        if items:
            log.warning("Graph pagination truncated after %d items: %s", len(items), exc)
        else:
            ...
            if hasattr(headers, "get"):
                ...
                if ra:
                    try:
                        ...
                        pass
            if items:
                log.warning("ARM 429 throttled after %d items — returning partial (retry-after=%ds)",
                            len(items), retry_after)
            else:
                log.warning("ARM 429 throttled with 0 items — sleeping %ds then re-raising",
                            retry_after)
                await asyncio.sleep(retry_after)
                raise
        elif items:
            log.warning("ARM pagination truncated after %d items: %s", len(items), exc)
        else:
