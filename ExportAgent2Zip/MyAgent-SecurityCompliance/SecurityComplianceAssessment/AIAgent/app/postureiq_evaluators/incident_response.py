...
def evaluate_incident_response(
    ...
    dispatch = {
        ...
        "check_sentinel_soc_maturity": _check_sentinel_soc_maturity,
    }
...
# ---------------------------------------------------------------------------
# v55 — Sentinel SOC Maturity
# ---------------------------------------------------------------------------
def _check_sentinel_soc_maturity(cid, ctrl, evidence, idx, thresholds=None):
    """Evaluate SOC maturity via Sentinel workbooks, hunting queries, and incident workflow."""
    findings = []
    workspaces = idx.get("azure-sentinel-workspace", [])
    connectors = idx.get("azure-sentinel-connector", [])
    rules = idx.get("azure-sentinel-rule", [])
    ...
    if not workspaces:
        return [_f(cid, ctrl, Status.NON_COMPLIANT,
                   "No Sentinel workspaces — SOC maturity cannot be assessed.",
                   recommendation="Enable Microsoft Sentinel for SIEM/SOAR capabilities.")]

    # Connector breadth (data source diversity)
    if connectors:
        kinds = {c.get("Data", {}).get("Kind", "").lower() for c in connectors}
        if len(kinds) < 3:
            findings.append(_f(cid, ctrl, Status.PARTIAL,
                              f"Only {len(kinds)} Sentinel connector type(s). SOC needs diverse data sources.",
                              recommendation="Add connectors for identity, endpoint, cloud, and network data sources."))
    else:
        findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                          "No Sentinel data connectors — SOC has no telemetry.",
                          recommendation="Configure data connectors for Office 365, Azure AD, Defender, and cloud resources."))

    # Analytics rule maturity
    if rules:
        enabled = [r for r in rules if r.get("Data", {}).get("Enabled") is not False]
        scheduled = [r for r in enabled if r.get("Data", {}).get("Kind", "").lower() == "scheduled"]
        if len(enabled) < 15:
            findings.append(_f(cid, ctrl, Status.PARTIAL,
                              f"SOC has {len(enabled)} analytics rules (recommend ≥15 for mature coverage).",
                              recommendation="Deploy additional analytics rule templates from the Sentinel content hub."))
    else:
        findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                          "No analytics rules — SOC cannot detect threats.",
                          recommendation="Deploy analytics rules for common attack techniques (MITRE ATT&CK)."))

    # Automation / SOAR readiness
    if not automation:
        findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                          "No Sentinel automation — incidents require fully manual triage.",
                          recommendation="Configure automation rules and Logic App playbooks for incident enrichment and response."))

    # Incident backlog health
    if incidents:
        open_incidents = [i for i in incidents
                          if i.get("Data", {}).get("Status", "").lower() in ("new", "active")]
        closed = len(incidents) - len(open_incidents)
        if open_incidents and closed == 0:
            findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                              f"{len(open_incidents)} open Sentinel incidents with none resolved — SOC may be non-operational.",
                              recommendation="Establish incident triage workflow. Assign analysts and set SLAs."))
        elif len(open_incidents) > 100:
            findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                              f"{len(open_incidents)} open incidents indicate backlog. Review rule tuning.",
                              recommendation="Tune analytics rules to reduce false positives and clear incident backlog."))

    if not findings:
        findings.append(_f(cid, ctrl, Status.COMPLIANT,
                          f"SOC maturity adequate: {len(workspaces)} workspace(s), {len(connectors)} connectors, "
                          f"{len(rules)} rules, {len(automation)} automations."))
    return findings
