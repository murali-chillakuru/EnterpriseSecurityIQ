...
from app.collectors.base import run_collector, paginate_arm, make_evidence, _v, _extract_status, ACCESS_DENIED_CODES
            ...
            try:
                ...
                for srv in servers:
                    ...
                    try:
                        ...
                        if _extract_status(exc) in ACCESS_DENIED_CODES:
                            log.warning("  [SqlDetailed] %s ATP access denied (403) — user lacks permission", srv.name)
                        else:
                            log.debug("  [SqlDetailed] %s ATP check unavailable: %s", srv.name, exc)
                    ...
                    try:
                        ...
                        if _extract_status(exc) in ACCESS_DENIED_CODES:
                            log.warning("  [SqlDetailed] %s vulnerability assessment access denied (403)", srv.name)
                        else:
                            log.debug("  [SqlDetailed] %s vulnerability assessment not configured: %s", srv.name, exc)
                    ...
                    try:
                        ...
                        if _extract_status(exc) in ACCESS_DENIED_CODES:
                            log.warning("  [SqlDetailed] %s firewall rules access denied (403)", srv.name)
                        else:
                            log.debug("  [SqlDetailed] %s firewall rules unavailable: %s", srv.name, exc)
                    allow_all = any(
                    ...
                    async def _check_db(db):
                        ...
                        try:
                            ...
                            if _extract_status(exc) in ACCESS_DENIED_CODES:
                                log.debug("  [SqlDetailed] %s/%s audit policy access denied", srv.name, db_name)
                        try:
                            async with _CONCURRENCY:
                            ...
                            if _extract_status(exc) in ACCESS_DENIED_CODES:
                                log.debug("  [SqlDetailed] %s/%s TDE access denied", srv.name, db_name)
                        return {
