                ...
                async def _fetch_extensions(vm):
                    ...
                    try:
                        ...
                        if _extract_status(exc) in ACCESS_DENIED_CODES:
                            log.debug("  [AzureCompute] VM %s extensions access denied", vm.name)
                    return vm, has_mde
                ...
                async def _fetch_sql_details(srv):
                    ...
                    async with _CONCURRENCY:
                        try:
                            ...
                            if _extract_status(exc) in ACCESS_DENIED_CODES:
                                log.debug("  [AzureCompute] SQL %s AD admin access denied", srv.name)
                        try:
                            if rg_name:
                            ...
                            if _extract_status(exc) in ACCESS_DENIED_CODES:
                                log.debug("  [AzureCompute] SQL %s audit policy access denied", srv.name)
                        try:
                            if rg_name:
                            ...
                            if _extract_status(exc) in ACCESS_DENIED_CODES:
                                log.debug("  [AzureCompute] SQL %s TDE access denied", srv.name)
