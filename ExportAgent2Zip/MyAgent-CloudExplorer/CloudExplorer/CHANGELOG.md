...
All notable enhancements, fixes, and deployment changes.

**Author:** Murali Chillakuru

---

## Infra Fix — Storage Account Public Network Access (2026-04-22)

**Problem:** All blob uploads from the container app were failing with `AuthorizationFailure` ("This request is not authorized to perform this operation"). Report View/Download links returned `"Report not found"` after container restarts because the blob storage fallback could never succeed.

**Root Cause:** `esiqnewstorage` had `publicNetworkAccess: Disabled`. The Container App environment (`ESIQNew-env`) has no VNet integration and no private endpoint to the storage account, so all connections were blocked at the network layer — before RBAC could even be evaluated.

**Fix:** Enabled public network access on the storage account:
```
az storage account update --name esiqnewstorage --resource-group ESIQNew-RG --public-network-access Enabled
```

**Security posture maintained:**
- `allowSharedKeyAccess: false` — only Entra ID / RBAC auth works (Azure Policy enforced)
- Managed identity `ESIQNew-identity` retains `Storage Blob Data Contributor` role
- No anonymous blob access
- No code changes required

**Impact:** Report persistence across container restarts/redeployments now works. Affects both Teams PostureIQ and standalone webapp.

---

## v93 — Progress Panel Stuck After Session Switch (2026-04-22)

**Problem:** After switching sessions, the progress panel (spinner + Stop button) remained visible indefinitely because `removeProgress()` relied on a cached `progressEl` variable that was set to `null` during session switch cleanup.

**Fix:** Added `document.getElementById('progressPanel')` fallback in `removeProgress()` across all 9 HTML files (7 assessment pages + CloudExplorer + PostureIQ + Teams PostureIQ). When the cached reference is stale, the function falls back to a direct DOM lookup.

**Files changed:** All 9 HTML files in `webapp/`

---

## v92 — Follow-Up Chip Buttons Not Clickable (2026-04-22)

**Problem:** Follow-up chip buttons stopped working after session restore. `innerHTML`-based session restore stripped inline `onclick` handlers from chip elements.

**Fix:** Replaced inline `onclick` with `data-prompt` attribute + delegated `document.addEventListener('click')` handler. Chips now work regardless of how they were inserted into the DOM.

**Files changed:** 7 assessment HTML files in `webapp/`
...
| **`webapp/SecurityComplianceAssessment.html`** | Updated sidebar, login, and welcome text with new capability descriptions |
...
| **`webapp/SecurityComplianceAssessment.html`** (new) | Focused SPA for PostureIQ posture assessments (with framework picker) |
