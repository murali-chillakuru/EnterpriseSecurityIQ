...
def _dedup_rows(rows: list[dict]) -> list[dict]:
    """Deduplicate result rows by resource identity.

    When multiple templates return the same resource, keep the row from the
    first (most specific) template.  Uses (name, resourceGroup, subscriptionId)
    as the composite key, falling back to (id) or (displayName) for Entra.
    """
    seen: set[tuple] = set()
    out: list[dict] = []
    for r in rows:
        ...
        key = (
            r.get("id")
            or (r.get("name", ""), r.get("resourceGroup", ""), r.get("subscriptionId", ""))
        )
...
def _build_result(
    ...
    """Build a standardized result dict with error separation and truncation info."""
    # Separate error rows from real results
    real_rows = [r for r in all_rows if "_error" not in r]
    error_rows = [r for r in all_rows if "_error" in r]
    errors.extend(error_rows)

    # Deduplicate across multi-match templates
    real_rows = _dedup_rows(real_rows)
    total = len(real_rows)
    cap = top * max(template_count, 1)
    display_rows = real_rows[:cap]
    ...
    if total > cap:
        ...
        result["truncation_note"] = (
            f"Showing {cap} of {total} results. "
            "Refine your query or add a region/subscription filter to see more."
        )
    # Errors — separated so the LLM can report them clearly
    if errors:
        result["errors"] = [
            {"template": e.get("_queryTemplate", "unknown"), "error": e.get("_error", "unknown")}
            for e in errors
        ]
    return result


async def _execute_arg_templates(
    creds: ComplianceCredentials,
    templates: list[str],
    top: int,
) -> tuple[list[dict], list[dict]]:
    """Execute a list of ARG templates.  Returns (rows, errors)."""
    all_rows: list[dict] = []
    errors: list[dict] = []
    for tpl in templates:
        kql = ARG_TEMPLATES.get(tpl)
        if not kql:
            errors.append({"_queryTemplate": tpl, "_error": f"Unknown template: {tpl}"})
            continue
        try:
            ...
            errors.append({"_queryTemplate": tpl, "_error": str(exc)})
...
async def dispatch_natural_language(
    ...
    all are executed, deduplicated, and merged.
    ...
    if matched_templates:
        ...
        all_rows, errors = await _execute_arg_templates(creds, matched_templates, top)

        source = "arg" if len(matched_templates) == 1 else "multi"
        return _build_result(source, ", ".join(matched_templates), all_rows, errors, top, len(matched_templates))
    ...
    for keywords, query_type in NL_ENTRA_MAP:
        if any(kw in q for kw in keywords):
            result = await _run_entra_query(creds, query_type, q, top)
            # Propagate Entra errors into the standardized format
            if result.get("error"):
                result["errors"] = [{"template": query_type, "error": result["error"]}]
            return result
    ...
    if llm_picks:
        ...
        all_rows, errors = await _execute_arg_templates(creds, arg_picks, top)
        ...
        for qt in entra_picks:
            try:
                ...
                if result.get("error"):
                    errors.append({"_queryTemplate": qt, "_error": result["error"]})
            except Exception as exc:
                log.warning("Entra query %s failed: %s", qt, exc)
                errors.append({"_queryTemplate": qt, "_error": str(exc)})

        source = "multi" if len(llm_picks) > 1 else ("arg" if arg_picks else "entra")
        return _build_result(source, ", ".join(llm_picks), all_rows, errors, top, len(llm_picks))

    # 5. Nothing matched — suggest closest templates using fuzzy match
    suggestions = _suggest_templates(question)
    msg = (
        "Could not match your query to any available data source. "
    ...
    if suggestions:
        result["suggested_templates"] = suggestions
        result["message"] += (
            "\n\nDid you mean one of these? "
            + ", ".join(s.replace("_", " ") for s in suggestions)
        )
    return result


def _suggest_templates(question: str, n: int = 5) -> list[str]:
    """Return up to *n* template names closest to the question words."""
    q_words = question.lower().split()
    # Build a flat list of all template names (ARG + Entra + composite)
    all_names = list(ARG_TEMPLATES.keys()) + list(_ENTRA_QUERY_TYPES) + list(COMPOSITE_NAMES)
    # Score each template by best word overlap with the query
