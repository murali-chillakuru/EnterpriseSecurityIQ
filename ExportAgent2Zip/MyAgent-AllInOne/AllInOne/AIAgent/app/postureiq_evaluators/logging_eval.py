...
def evaluate_logging(
    ...
    dispatch = {
        ...
        "check_sentinel_operational_maturity": _check_sentinel_operational_maturity,
    }
...
# ---------------------------------------------------------------------------
# v55 — Sentinel Operational Maturity (UEBA, TI, Playbooks, Workbooks)
# ---------------------------------------------------------------------------
def _check_sentinel_operational_maturity(cid, ctrl, evidence, idx, thresholds=None):
    """Evaluate Sentinel operational maturity: UEBA, threat intelligence, playbooks, workbooks."""
    findings = []
    ...
    if not workspaces:
        return [_f(cid, ctrl, Status.NON_COMPLIANT,
                   "No Sentinel workspaces — operational maturity cannot be assessed.",
                   recommendation="Enable Microsoft Sentinel on a Log Analytics workspace.")]

    for item in workspaces:
        d = item.get("Data", {})
        name = d.get("Name", "unknown")
        r = _res(item, "Microsoft.OperationalInsights/workspaces")

        # UEBA enablement
        ueba = d.get("UebaEnabled") or d.get("EntityBehaviorAnalytics")
        if not ueba:
            findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                              f"Sentinel workspace '{name}' does not have UEBA (Entity Behavior Analytics) enabled.",
                              recommendation="Enable UEBA for user and entity anomaly detection.", **r))

        # Threat intelligence connectors
        ti_connectors = [c for c in connectors
                         if "threatintelligence" in (c.get("Data", {}).get("Kind") or "").lower()
                         or "ti" in (c.get("Data", {}).get("Kind") or "").lower()]
        if not ti_connectors:
            findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                              f"Sentinel workspace '{name}' has no threat intelligence connectors.",
                              recommendation="Configure Threat Intelligence connector for IoC enrichment.", **r))

    # Playbook / automation coverage
    if not automation:
        findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                          "No Sentinel automation rules or playbooks configured.",
                          recommendation="Create automation rules with Logic App playbooks for automated incident response."))
    else:
        enabled_auto = [a for a in automation if a.get("Data", {}).get("Enabled") is not False]
        if not enabled_auto:
            findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                              f"All {len(automation)} Sentinel automation rules are disabled.",
                              recommendation="Enable automation rules for automated incident triage and response."))

    # Analytics rules coverage
    if rules:
        enabled_rules = [r for r in rules if r.get("Data", {}).get("Enabled") is not False]
        if len(enabled_rules) < 10:
            findings.append(_f(cid, ctrl, Status.PARTIAL,
                              f"Only {len(enabled_rules)} Sentinel analytics rules enabled (recommend ≥10).",
                              recommendation="Deploy additional analytics rule templates for broader threat coverage."))
    else:
        findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                          "No Sentinel analytics rules configured.",
                          recommendation="Deploy analytics rules from the Sentinel content hub."))

    if not findings:
        findings.append(_f(cid, ctrl, Status.COMPLIANT,
                          f"Sentinel operational maturity: {len(workspaces)} workspace(s), "
                          f"{len(connectors)} connectors, {len(rules)} rules, {len(automation)} automations."))
    return findings
