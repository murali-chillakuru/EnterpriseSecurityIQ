...
def evaluate_network(
    ...
    dispatch = {
        ...
        # v55 — expanded network posture
        "check_ddos_protection": _check_ddos_protection,
        "check_bastion_adoption": _check_bastion_adoption,
        "check_vpn_expressroute_security": _check_vpn_expressroute_security,
        "check_nat_gateway_security": _check_nat_gateway_security,
        "check_vwan_security": _check_vwan_security,
    }
...
# v55 — DDoS Protection
# ---------------------------------------------------------------------------
def _check_ddos_protection(cid, ctrl, evidence, idx):
    """Check that DDoS Protection plans exist and VNets are associated."""
    findings = []
    vnets = idx.get("azure-virtual-network", [])
    # DDoS plans collected as azure-resource with type containing 'ddosprotection'
    ddos_plans = [r for r in idx.get("azure-resource", [])
                  if "ddosprotection" in (r.get("Data", {}).get("ResourceType") or "").lower()]

    if not ddos_plans:
        public_ips = [v for v in vnets if v.get("Data", {}).get("HasPublicIPs")]
        if vnets:
            findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                              f"No DDoS Protection plan found. {len(vnets)} VNet(s) may be unprotected against volumetric attacks.",
                              recommendation="Enable Azure DDoS Network Protection on VNets with public-facing resources."))
    else:
        # Check VNets without DDoS enabled
        for item in vnets:
            d = item.get("Data", {})
            name = d.get("Name", "unknown")
            r = _res(item, "Microsoft.Network/virtualNetworks")
            ddos_enabled = d.get("DdosProtectionEnabled") or d.get("EnableDdosProtection")
            if not ddos_enabled:
                findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                                  f"VNet '{name}' does not have DDoS protection enabled.",
                                  recommendation="Associate VNet with a DDoS Protection plan.", **r))

    if not findings:
        findings.append(_f(cid, ctrl, Status.COMPLIANT,
                          f"DDoS protection configured ({len(ddos_plans)} plan(s), {len(vnets)} VNet(s))."))
    return findings


# ---------------------------------------------------------------------------
# v55 — Bastion Adoption (secure management access)
# ---------------------------------------------------------------------------
def _check_bastion_adoption(cid, ctrl, evidence, idx):
    """Check Bastion hosts exist for secure VM management (no public RDP/SSH)."""
    findings = []
    bastions = [r for r in idx.get("azure-resource", [])
                if "bastion" in (r.get("Data", {}).get("ResourceType") or "").lower()]
    nsg_rules = idx.get("azure-nsg-rule", [])

    # Check for open RDP/SSH ports
    mgmt_open = [rule for rule in nsg_rules
                 if rule.get("Data", {}).get("DestPort") in ("3389", "22", "*")
                 and rule.get("Data", {}).get("Access", "").lower() == "allow"
                 and rule.get("Data", {}).get("Direction", "").lower() == "inbound"
                 and rule.get("Data", {}).get("SourcePrefix") in ("*", "0.0.0.0/0", "Internet")]

    if mgmt_open and not bastions:
        findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                          f"{len(mgmt_open)} NSG rules expose RDP/SSH to internet but no Azure Bastion hosts deployed.",
                          recommendation="Deploy Azure Bastion to eliminate public RDP/SSH. Remove inbound 3389/22 rules from NSGs."))
    elif not bastions:
        findings.append(_f(cid, ctrl, Status.PARTIAL,
                          "No Azure Bastion hosts detected. Consider Bastion for secure VM management.",
                          recommendation="Deploy Azure Bastion to provide secure, auditable RDP/SSH access without public IPs."))

    if not findings:
        findings.append(_f(cid, ctrl, Status.COMPLIANT,
                          f"{len(bastions)} Bastion host(s) deployed for secure management access."))
    return findings


# ---------------------------------------------------------------------------
# v55 — VPN / ExpressRoute Security
# ---------------------------------------------------------------------------
def _check_vpn_expressroute_security(cid, ctrl, evidence, idx):
    """Evaluate VPN gateway and ExpressRoute security settings."""
    findings = []
    gateways = [r for r in idx.get("azure-resource", [])
                if "virtualnetworkgateway" in (r.get("Data", {}).get("ResourceType") or "").lower()
                or "expressroutecircuit" in (r.get("Data", {}).get("ResourceType") or "").lower()]
    ...
    for item in gateways:
        ...
        # VPN: check for IKEv2 and non-Basic SKU
        if "virtualnetworkgateway" in rtype.lower():
            sku = d.get("Sku", d.get("GatewaySku", ""))
            if isinstance(sku, dict):
                sku = sku.get("name", sku.get("tier", ""))
            if sku and "basic" in str(sku).lower():
                findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                                  f"VPN Gateway '{name}' uses Basic SKU (no IKEv2, no BGP, limited throughput).",
                                  recommendation="Upgrade to VpnGw1 or higher for IKEv2 and zone redundancy support.", **r))

        # ExpressRoute: check for private peering
        if "expressroute" in rtype.lower():
            state = d.get("ServiceProviderProvisioningState") or d.get("CircuitProvisioningState", "")
            if str(state).lower() not in ("provisioned", "enabled"):
                findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                                  f"ExpressRoute circuit '{name}' is not in provisioned state (state: {state}).",
                                  recommendation="Complete ExpressRoute provisioning with your service provider.", **r))

    if not findings:
        if gateways:
            findings.append(_f(cid, ctrl, Status.COMPLIANT,
                              f"All {len(gateways)} VPN/ExpressRoute gateway(s) pass security checks."))
        else:
            findings.append(_f(cid, ctrl, Status.COMPLIANT,
                              "No VPN/ExpressRoute gateways to evaluate."))
    return findings


# ---------------------------------------------------------------------------
# v55 — NAT Gateway Security
# ---------------------------------------------------------------------------
def _check_nat_gateway_security(cid, ctrl, evidence, idx):
    """Check NAT Gateways have public IPs and are associated to subnets."""
    findings = []
    nat_gws = [r for r in idx.get("azure-resource", [])
               if "natgateway" in (r.get("Data", {}).get("ResourceType") or "").lower()]

    for item in nat_gws:
        d = item.get("Data", {})
        name = d.get("Name", "unknown")
        r = _res(item, "Microsoft.Network/natGateways")
        public_ips = d.get("PublicIpAddresses") or d.get("PublicIPAddressCount", 0)
        subnets = d.get("Subnets") or d.get("SubnetCount", 0)
        ...
        if not subnets:
            findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                              ...
                              recommendation="Associate NAT Gateway with subnets for outbound connectivity.", **r))

    if not findings:
        findings.append(_f(cid, ctrl, Status.COMPLIANT,
                          f"NAT Gateway configuration adequate ({len(nat_gws)} gateway(s))."))
    return findings


# ---------------------------------------------------------------------------
# v55 — Virtual WAN Security
# ---------------------------------------------------------------------------
def _check_vwan_security(cid, ctrl, evidence, idx):
    """Evaluate Virtual WAN and Virtual Hub security posture."""
    findings = []
    vwans = [r for r in idx.get("azure-resource", [])
             if "virtualwan" in (r.get("Data", {}).get("ResourceType") or "").lower()
             or "virtualhub" in (r.get("Data", {}).get("ResourceType") or "").lower()]
    ...
    for item in vwans:
        ...
        if "virtualhub" in rtype.lower():
            # Check hub has a firewall or routing intent
            has_firewall = d.get("AzureFirewall") or d.get("SecurityProvider")
            routing_intent = d.get("RoutingIntent") or d.get("RoutingState")
            if not has_firewall and not routing_intent:
                findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                                  f"Virtual Hub '{name}' has no secured hub (no firewall or routing intent).",
                                  recommendation="Enable secured hub with Azure Firewall or routing intent for traffic inspection.", **r))

    if not findings:
        if vwans:
            findings.append(_f(cid, ctrl, Status.COMPLIANT,
                              f"Virtual WAN security adequate ({len(vwans)} resource(s))."))
        else:
            findings.append(_f(cid, ctrl, Status.COMPLIANT, "No Virtual WAN resources to evaluate."))
    return findings
