...
class ResourceInventory:
    ...
    async def ensure_loaded(self, creds: Any, subscriptions: list[dict]) -> None:
        ...
        Reloads automatically if the cache is older than _INVENTORY_TTL."""
        async with self._lock:
            if self._loaded and (time.monotonic() - self._loaded_at) < _INVENTORY_TTL:
                return
            if self._loaded:
                log.info("ResourceInventory: cache expired (>%d min) — reloading",
                         _INVENTORY_TTL // 60)
