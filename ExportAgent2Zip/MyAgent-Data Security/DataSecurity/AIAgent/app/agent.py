"""
PostureIQ Agent — Tool definitions and system prompt.
...
async def _generate_ds_reports(results: dict) -> str:
    ...
    try:
        ...
        try:
            ...
            log.warning("Executive brief generation skipped: %s", exc)
...
async def _auto_preflight(tool_name: str) -> str | None:
    ...
    try:
        ...
        # Don't block the assessment, but surface the preflight failure
        # as a warning the LLM can relay to the user.
        log.warning("[%s] Pre-flight check errored (non-fatal, proceeding): %s", tool_name, exc, exc_info=True)
        return (
            f"## ⚠️ Permission Pre-Check Could Not Complete\n\n"
            f"The automatic permission pre-flight for **{tool_name.replace('_', ' ').title()}** "
            f"encountered an error: `{exc}`\n\n"
            f"The assessment will proceed, but some collectors may fail if permissions are "
            f"insufficient. If you see errors, run **Check Permissions** from the sidebar.\n\n"
            f"---\n\n"
        )


SYSTEM_PROMPT = """You are PostureIQ, an AI-powered security posture assessment and tenant intelligence agent for Microsoft Azure and Entra ID environments.
...
5. **NEVER suggest the user run PowerShell/CLI commands** for data that an available PostureIQ tool can provide.
...
async def get_session_context_summary() -> str | None:
    ...
    if piq:
        ...
        parts.append(
            ...
            f"Use query_results to list findings, attack paths, priorities, quick wins, "
            f"or exposure details (public storage, open NSGs, unencrypted VMs)."
        )

    # Exposure scan results
    expo = state.get("exposure_results")
    if expo:
        parts.append(f"• Exposure Scan: {expo.get('total_exposed', 0)} exposed resources found "
                      f"(category={expo.get('category', 'all')}, scanned {expo.get('scan_time', 'N/A')}). "
                      f"Use query_results to review specific exposed resources from the cached scan.")
...
_TS_FAMILY_STORAGE = {
    "storage_accounts", "storage_public_access", "data_lake_stores",
    "file_shares", "netapp_accounts", "elastic_sans",
    "managed_disks_encryption",
}
_TS_FAMILY_COMPUTE = {
    "all_vms", "spot_vms", "vmss", "dedicated_hosts",
    "availability_sets", "disk_overview", "vm_extensions",
    "images_snapshots", "capacity_reservations", "compute_galleries",
    "baremetal_instances", "automanage",
}
_TS_FAMILY_NETWORK = {
    "public_ips", "all_nsgs", "nsg_open_rules", "vnets_subnets",
    "private_endpoints", "front_door", "expressroute", "vpn_gateways",
    "bastion_hosts", "firewalls", "firewall_policies", "ddos_protection",
    "virtual_wan", "traffic_manager", "nat_gateways", "network_watchers",
    "nsg_flow_logs", "ip_groups", "peerings", "route_servers",
    "service_endpoints", "application_security_groups", "load_balancers",
    "app_gateways", "dns_zones", "dns_private_resolvers",
}
_TS_FAMILY_IDENTITY = {
    "users", "disabled_users", "guest_users", "admin_users",
    "directory_roles", "conditional_access", "apps", "service_principals",
    "groups", "stale_users", "risky_users", "named_locations",
    "auth_methods", "pim_eligible", "organization_info",
    "security_defaults", "risk_detections", "risky_service_principals",
    "access_reviews", "consent_grants", "federated_credentials",
    "cross_tenant_access", "sharepoint_sites", "sensitivity_labels",
    "dlp_policies",
}
_TS_FAMILY_SECURITY = {
    "defender_plans", "security_recommendations", "secure_score",
    "policy_compliance", "regulatory_compliance", "jit_policies",
    "adaptive_app_controls", "defender_auto_provisioning",
    "defender_assessments", "defender_alerts", "defender_easm",
}
_TS_FAMILY_GOVERNANCE = {
    "subscriptions", "resource_groups", "management_groups",
    "resource_counts_by_subscription", "resources_by_subscription",
    "policy_assignments", "resource_locks", "custom_roles",
    "deny_assignments", "blueprint_assignments", "policy_exemptions",
    "policy_definitions_custom", "lighthouse_registrations",
    "role_assignments", "hierarchy_tree", "resource_drill_down",
}
_TS_FAMILY_MONITORING = {
    "diagnostic_settings", "log_analytics_workspaces", "alert_rules",
    "application_insights", "managed_grafana", "managed_prometheus",
    "action_groups", "service_health", "sentinel_workspaces",
    "sentinel_data_connectors", "health_models",
}
_TS_FAMILY_DATABASE = {
    "sql_servers", "sql_databases_detailed", "sql_firewall_rules",
    "sql_managed_instances", "sql_virtual_machines", "postgres_mysql",
    "mariadb", "elastic_pools", "cosmosdb", "redis",
}
_TS_FAMILY_AI = {
    "openai_deployments", "ml_workspaces", "ai_services",
    "search_services", "purview_accounts", "avs_private_clouds",
}
_TS_FAMILY_APP = {
    "webapp_detailed", "function_apps", "static_web_apps",
    "app_service_plans", "app_service_environments", "spring_apps",
    "app_configuration", "apim", "logic_apps", "event_grid",
    "event_hubs", "service_bus", "relay_namespaces", "signalr",
    "notification_hubs", "communication_services", "cdn_profiles",
    "media_services",
}
_TS_FAMILY_CONTAINERS = {
    "aks_clusters", "container_apps", "container_instances",
    "container_registries", "aro_clusters", "arc_kubernetes",
    "managed_environments",
}


def _tenant_search_family(
    source: str,
    query_templates: list[str],
    field_names: list[str],
    question: str,
) -> str:
    """Map a tenant search result to a coarse family for UI follow-up chips."""
    if source == "entra":
        return "identity"
    if source in {"resource_detail", "arg_raw"}:
        return "detail"

    for template in query_templates:
        if template in _TS_FAMILY_STORAGE:
            return "storage"
        if template in _TS_FAMILY_COMPUTE:
            return "compute"
        if template in _TS_FAMILY_NETWORK:
            return "network"
        if template in _TS_FAMILY_IDENTITY:
            return "identity"
        if template in _TS_FAMILY_SECURITY:
            return "security"
        if template in _TS_FAMILY_GOVERNANCE:
            return "governance"
        if template in _TS_FAMILY_MONITORING:
            return "monitoring"
        if template in _TS_FAMILY_DATABASE:
            return "database"
        if template in _TS_FAMILY_AI:
            return "ai"
        if template in _TS_FAMILY_APP:
            return "app"
        if template in _TS_FAMILY_CONTAINERS:
            return "containers"

    fields = {f.lower() for f in field_names}
    q = question.lower()
    if {"userprincipalname", "accountenabled", "lastsigndatetime"} & fields:
        return "identity"
    if {"resourcegroup", "subscriptionid", "location"} <= fields and "storage" in q:
        return "storage"
    if {"resourcegroup", "subscriptionid", "location"} <= fields and "network" in q:
        return "network"
    if {"risklevel", "riskstate"} & fields:
        return "security"
    return "inventory"


def _extract_tenant_search_field_names(rows: list[dict]) -> list[str]:
    names: set[str] = set()
    for row in rows[:10]:
        ...
        for key in row.keys():
            key_s = str(key)
            if not key_s.startswith("_"):
                names.add(key_s)
    return sorted(names)[:30]


def _build_tenant_search_meta(
    question: str,
    *,
    source: str,
    query_used: str = "",
    ...
    query_templates = [part.strip() for part in query_used.split(",") if part.strip()]
    field_names = _extract_tenant_search_field_names(rows)

    if errors and not rows:
        state = "error"
    elif source == "none":
        state = "no_match" if suggested_templates else "empty"
    elif not rows:
        state = "empty"
    else:
        state = "results"
    ...
    return {
        ...
        "result_family": _tenant_search_family(source, query_templates, field_names, question),
        "count": count,
        ...
        "has_tags": "tags" in field_names,
        "has_subscription": "subscriptionId" in field_names,
    }


async def get_last_tenant_search_meta() -> dict | None:
    """Return the most recent Cloud Explorer result metadata for this conversation."""
    state = await _get_session_state()
    meta = state.get("last_tenant_search_meta") if state else None
...
async def query_results(
    ...
    if piq and any(kw in q for kw in ["postureiq", "posture", "risk score", "risk tier",
                                        ...
                                        "exposure", "exposed", "public storage", "open nsg",
                                        "unencrypted", "unattached disk", "public ip",
                                        "storage account", "internet facing"]):
        piq_summary = piq.get("summary", {})
    ...
    # ── Exposure scan results ──
    expo = state.get("exposure_results")
    if expo and any(kw in q for kw in ["exposure", "exposed", "public storage", "open nsg",
                                         "unencrypted", "unattached", "public ip",
                                         "public access", "internet facing", "open port"]):
        details = expo.get("details_md", "")
        if details:
            return details
        return (f"Exposure scan ran (category={expo.get('category', 'all')}, "
                f"{expo.get('total_exposed', 0)} exposed resources). "
                f"Re-run search_exposure for full details.")
...
async def search_tenant(
    ...
    try:
        ...
        if question.strip().lower().startswith("/arg "):
            ...
            meta = _build_tenant_search_meta(
                question,
                source="arg_raw",
                query_used="raw_arg",
                rows=rows,
            ...
            await _update_session_state("last_tenant_search_meta", meta)
            if not rows:
                return "No resources found matching the KQL query."
            # Build subscription name map for friendly display
            sub_ids_raw = {r.get("subscriptionId") for r in rows if r.get("subscriptionId")}
            sub_map_raw: dict[str, str] = {}
            if sub_ids_raw:
                try:
                    all_subs_raw = await creds.list_subscriptions()
                    sub_map_raw = {s["subscription_id"]: s["display_name"] for s in all_subs_raw}
                except Exception:
                    pass
            result_md = f"## ARG Query Results ({len(rows)} resources)\n\n"
            for row in rows[:30]:
                sub_id = row.get("subscriptionId", "")
                sub_name = sub_map_raw.get(sub_id, sub_id) if sub_id else ""
                result_md += f"- **{row.get('name', 'N/A')}** ({row.get('type', '')}) "
                result_md += f"[{row.get('location', '')}] {row.get('resourceGroup', '')}"
                if sub_name:
                    result_md += f" | Sub: {sub_name}"
                result_md += "\n"
            if len(rows) > 30:
        ...
        if question.strip().startswith("/resource "):
            ...
            meta = _build_tenant_search_meta(
                ...
                query_used="resource_detail",
                rows=[detail] if isinstance(detail, dict) and "error" not in detail else [],
                count=0 if "error" in detail else 1,
                errors=[{"template": "resource_detail", "error": detail["error"]}] if "error" in detail else [],
            )
            await _update_session_state("last_tenant_search_meta", meta)
            if "error" in detail:
        ...
        if question.strip().startswith("/user "):
            ...
            meta = _build_tenant_search_meta(
                question,
                source="resource_detail",
                query_used="user_detail",
                rows=[detail] if isinstance(detail, dict) and "error" not in detail else [],
                count=0 if "error" in detail else 1,
                errors=[{"template": "user_detail", "error": detail["error"]}] if "error" in detail else [],
            )
            await _update_session_state("last_tenant_search_meta", meta)
            if "error" in detail:
                return f"Error: {detail['error']}"
        ...
        meta = _build_tenant_search_meta(
            ...
            truncation_note=result.get("truncation_note", ""),
        ...
        if not rows:
            # Distinguish real empty from query errors
            if result.get("errors"):
                error_msgs = "; ".join(
                    f"{e['template']}: {e['error']}" for e in result["errors"]
                )
                return f"Query executed but encountered errors:\n{error_msgs}\n\nPlease check permissions or try a more specific query."
            return f"Query executed ({result.get('query_used', '')}) but returned no results. The resources may not exist in the queried subscriptions, or you may need Reader access."
        ...
        if source == "findings":
            ...
            # Build subscription name map so the LLM can show friendly names
            sub_ids = {row.get("subscriptionId") for row in rows if row.get("subscriptionId")}
            sub_name_map: dict[str, str] = {}
            if sub_ids:
                try:
                    all_subs = await creds.list_subscriptions()
                    sub_name_map = {s["subscription_id"]: s["display_name"] for s in all_subs}
                except Exception:
                    pass  # Fall back to showing subscriptionId as-is
            for row in rows[:30]:
                ...
                sub_id = row.get("subscriptionId", "")
                sub_display = sub_name_map.get(sub_id, sub_id) if sub_id else ""
                line = f"- **{name}** ({rtype}) [{loc}] {rg}"
                if sub_display:
                    line += f" | Sub: {sub_display}"
        ...
        # Surface truncation warning from dispatcher
        if result.get("truncated"):
            result_md += f"\n> **Note:** {result.get('truncation_note', 'Results may be truncated. Refine your query to see more.')}\n"

        # Surface query errors so the user knows what went wrong
        if result.get("errors"):
            result_md += "\n**Errors encountered:**\n"
            for err in result["errors"]:
                result_md += f"- {err.get('template', '?')}: {err.get('error', 'unknown')}\n"
...
async def run_attack_path_detection(
    min_severity: Annotated[
        str,
        "Minimum severity to include in the report (informational, low, medium, high, critical). Default 'informational' returns all detected paths.",
    ] = "informational",
    subscriptions: Annotated[
        str,
        "Optional comma-separated subscription IDs to limit the scan to. Leave empty to include all accessible subscriptions.",
    ] = "",
) -> str:
    """Detect cross-domain attack paths (consent abuse, privilege escalation, lateral movement, credential chains, exposed high-value resources) with MITRE ATT&CK mapping and per-step playbooks."""
    try:
        # ── Session-duplicate guard ──
        dup = await _session_duplicate_guard("run_attack_path_detection")
        if dup:
            return dup

        # ── Auto pre-flight permission check ──
        preflight_abort = await _auto_preflight("run_attack_path_detection")
        if preflight_abort:
            return preflight_abort

        sev = (min_severity or "informational").strip().lower()
        if sev not in {"informational", "low", "medium", "high", "critical"}:
            sev = "informational"

        log.info("[run_attack_path_detection] Starting (min_severity=%s, subs=%s) …", sev, subscriptions or "all")
        creds = _get_creds()
        tenant_id = getattr(creds, "tenant_id", None) or os.environ.get("AZURE_TENANT_ID", "")
        if not tenant_id:
            return "Attack path detection failed: tenant_id is not available from the current credentials."

        # Use a controlled output directory under the canonical /reports root
        out_dir = _make_out_dir("Attack-Path-Detection")

        from app.attack_path_engine import run_attack_path_assessment
        result = await run_attack_path_assessment(
            tenant_id=tenant_id,
        ...
        try:
            await _update_session_state("attack_path_results", result)
        except Exception:
            pass

        summary = (result or {}).get("Summary", {}) if isinstance(result, dict) else {}
        total_paths = summary.get("TotalPaths", 0)
        sev_counts = summary.get("SeverityCounts", {}) or {}
        overall_sev = str(summary.get("OverallSeverity", "unknown")).upper()
        overall_score = summary.get("OverallRiskScore", 0)
        ...
        # Build markdown links via _report_url helper
        links = ""
        for fname in ("attack-path-report.html", "attack-path-executive-brief.html", "attack-path-report.xlsx", "attack-path-assessment.json"):
            fp = out_dir / fname
            if fp.exists():
                links += f"- [{fp.name}]({_report_url(str(fp))})\n"

        try:
            _blob_upload_dir(out_dir, _OUTPUT_DIR)
        except Exception as exc:
            log.warning("Blob upload skipped: %s", exc)

        md = "## Attack Path Detection Complete\n\n"
        md += f"**Overall Risk Score: {overall_score}/100 ({overall_sev})** | "
        md += f"Total Paths: {total_paths}\n\n"
        md += f"**Severity breakdown:** Critical: {critical} \u00b7 High: {high} \u00b7 Medium: {medium} \u00b7 Low: {low} \u00b7 Informational: {info}\n\n"
        if links:
            md += "### Reports\n" + links + "\n"
        md += "_Full detection results cached \u2014 ask follow-up questions to explore specific paths, MITRE techniques, or remediation playbooks._\n"
        return md

    except Exception as exc:
        log.error("Attack path detection failed: %s", exc, exc_info=True)
        return f"Attack path detection failed: {exc}"
...
async def check_permissions() -> str:
    ...
    try:
        ...
        # Azure RBAC Role Assignments section
        rbac = result.get("rbac_assignments", [])
        if rbac:
            md += f"\n### Azure RBAC Role Assignments ({len(rbac)})\n\n"
            md += "| Role | Scope Level | Subscription | Scope |\n"
            md += "|---|---|---|---|\n"
            for ra in rbac:
                # Truncate long scope paths for readability
                scope_short = ra['scope']
                if len(scope_short) > 80:
                    scope_short = "…" + scope_short[-75:]
                md += f"| {ra['role']} | {ra['scope_level']} | {ra['subscription']} | `{scope_short}` |\n"

            # Summary by role
            role_counts: dict[str, int] = {}
            for ra in rbac:
                role_counts[ra['role']] = role_counts.get(ra['role'], 0) + 1
            md += "\n**Role Summary:** "
            md += ", ".join(f"{role} ({count})" for role, count in sorted(role_counts.items(), key=lambda x: -x[1]))
            md += "\n"

            # Subscription coverage
            sub_names = sorted(set(ra['subscription'] for ra in rbac))
            md += f"\n**Subscription Coverage:** {len(sub_names)} subscription(s) — {', '.join(sub_names)}\n"
        else:
            md += f"\n### Azure RBAC Role Assignments\n\nNo RBAC role assignments found for your identity.\n"
...
async def compare_runs(
    ...
    # Extract the actual PostureIQ results from session (not the full session dict)
    current_data = state.get("postureiq_results")
    if not current_data:
        return "No PostureIQ assessment results in session. Please run a SecurityComplianceAssessment first."
    ...
    try:
        ...
        # Fallback: try blob-based history if local filesystem has nothing
        if not prev_data:
            try:
                from app.evidence_history import list_runs, load_run
                creds = _get_creds()
                tenant_info = await creds.get_tenant_info()
                tid = tenant_info.get("tenant_id", "unknown")
                runs = list_runs(tid, limit=10)
                if len(runs) >= 2:
                    # runs are newest-first; skip index 0 (current), take index 1
                    prev_ts = runs[1]["timestamp"]
                    prev_data = load_run(tid, prev_ts)
                    log.info("[compare_runs] Loaded previous run from blob history: %s", prev_ts)
                elif len(runs) == 1:
                    return (
                        "Only one assessment run exists in history. "
                        "Run another assessment after making changes, then compare."
                    )
            except Exception as hist_exc:
                log.warning("[compare_runs] Blob history fallback failed: %s", hist_exc)

        if not prev_data:
            return "No previous assessment run found. Run at least two assessments to compare."

        delta = compute_delta(current_data, prev_data)
    ...
    try:
        ...
        for fw_key in fw_list:
            ...
            for fp in fw_flat:
                ...
                url = _report_url(fp)
                if ext == ".html":
                    row["html"] = url; row["html_name"] = p.name; fw_dir = str(p.resolve().parent)
                elif ext == ".pdf":
            ...
            # Filesystem fallback: look for a PDF sibling of the HTML file
            if "pdf" not in row and fw_dir:
                fw_dir_path = pathlib.Path(fw_dir)
                if fw_dir_path.is_dir():
                    for pdf_file in fw_dir_path.glob("*.pdf"):
                        row["pdf"] = _report_url(str(pdf_file))
                        row["pdf_name"] = pdf_file.name
                        break
...
TOOLS = [query_results, search_tenant, analyze_risk, assess_data_security, run_attack_path_detection, generate_rbac_report, generate_report, assess_copilot_readiness, assess_ai_agent_security, check_permissions, compare_runs, search_exposure, generate_custom_report, run_postureiq_assessment, query_assessment_history]
