...
async def _pp_get(session: aiohttp.ClientSession, url: str, token: str,
                  _max_retries: int = 3) -> dict | list | None:
    """Make an authenticated GET request to Power Platform admin API.
    Retries on 429 (throttled) with Retry-After backoff."""
        ...
        async with session.get(url, headers=headers) as resp:
            ...
            if resp.status == 429:
                ...
                if attempt < _max_retries:
                    log.warning("Power Platform 429 throttled (attempt %d/%d), retrying in %ds",
                                attempt, _max_retries, retry_after)
                    await asyncio.sleep(retry_after)
                    continue
                log.warning("Power Platform 429 — retries exhausted for %s", url)
                return None
