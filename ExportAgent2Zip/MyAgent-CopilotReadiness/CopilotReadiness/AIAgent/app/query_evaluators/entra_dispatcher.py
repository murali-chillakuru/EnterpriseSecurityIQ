...
async def _run_entra_query(
    ...
    try:
        if query_type == "disabled_users":
            ...
            # Try server-side filter first (requires AuditLog.Read.All)
            try:
                results = await query_entra_users(
                    creds,
                    filter_expr=f"signInActivity/lastSignInDateTime le {cutoff_str}",
                    top=top,
                )
            except Exception:
                # Fallback: fetch all users with higher page size and filter locally
                all_users = await query_entra_users(creds, top=max(top * 5, 999))
                results = []
                for u in all_users:
