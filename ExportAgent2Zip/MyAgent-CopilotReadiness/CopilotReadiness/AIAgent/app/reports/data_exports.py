...
def save_raw_evidence(evidence: list[dict], output_dir: str) -> list[str]:
    """Save raw evidence to raw/ subdirectory as JSON files grouped by evidence type.

    Returns a list of individual file paths (not the directory) so that
    ``_flatten_report_paths`` can pick them up for the SPA and ZIP bundle.
    """
