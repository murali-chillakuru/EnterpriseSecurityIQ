...
ARG_TEMPLATES: dict[str, str] = {
    ...
    "vms_without_disk_encryption": """
        ...
        | extend diskEncryption = properties.securityProfile.encryptionAtHost
        | where isnull(encryption) and diskEncryption != true
        | project name, resourceGroup, location,
                  hasEncryptionSet=isnotnull(encryption),
    ...
    "spot_vms": """
        Resources
        | where type =~ 'microsoft.compute/virtualmachines'
        | where tostring(properties.priority) =~ 'Spot'
        | project name, resourceGroup, location,
    ...
    "storage_public_access": """
        ...
        | where properties.allowBlobPublicAccess == true
               ...
               or properties.networkAcls.bypass contains 'AzureServices'
               or properties.networkAcls.defaultAction == 'Allow'
        | extend exposureReason = case(
               properties.allowBlobPublicAccess == true, 'BlobPublicAccess',
               properties.publicNetworkAccess == 'Enabled', 'PublicNetworkAccess',
               properties.networkAcls.defaultAction == 'Allow', 'FirewallAllowAll',
               properties.networkAcls.bypass contains 'AzureServices', 'AzureServicesBypass',
               'Other')
        | project name, resourceGroup, location,
                  allowBlobPublicAccess=properties.allowBlobPublicAccess,
                  publicNetworkAccess=properties.publicNetworkAccess,
                  firewallDefault=properties.networkAcls.defaultAction,
                  firewallBypass=properties.networkAcls.bypass,
                  exposureReason, subscriptionId
    """,
    "nsg_open_rules": """
        Resources
    ...
    "policy_compliance": """
        ...
        | where properties.complianceState in ('NonCompliant', 'Unknown')
        | summarize nonCompliantCount=countif(properties.complianceState == 'NonCompliant'),
                   unknownCount=countif(properties.complianceState == 'Unknown')
    ...
    "automanage": """
        ...
        | project name, resourceGroup, location,
                  configurationProfile=tostring(properties.configurationProfile),
    ...
    "compute_galleries": """
        ...
        | where type =~ 'microsoft.compute/galleries'
              or type =~ 'microsoft.compute/galleries/images'
              or type =~ 'microsoft.compute/galleries/images/versions'
        | project name, type, resourceGroup, location,
    ...
    "avs_private_clouds": """
        ...
        | project name, resourceGroup, location,
                  sku=sku.name,
                  provisioningState=properties.provisioningState,
                  internet=properties.internet,
                  subscriptionId
    """,
    "baremetal_instances": """
        Resources
        | where type =~ 'microsoft.baremetalinfrastructure/baremetalinstances'
        | project name, resourceGroup, location,
                  hardwareType=properties.hardwareType,
    ...
    "capacity_reservations": """
        ...
        | where type =~ 'microsoft.compute/capacityreservationgroups'
              or type =~ 'microsoft.compute/capacityreservationgroups/capacityreservations'
        | project name, type, resourceGroup, location,
                  sku=sku.name,
                  subscriptionId
    """,
    ...
    "route_servers": """
        Resources
        | where type =~ 'microsoft.network/virtualrouters'
        | project name, resourceGroup, location,
                  hostedSubnet=tostring(properties.hostedSubnet.id),
                  virtualRouterAsn=properties.virtualRouterAsn,
                  subscriptionId
    """,
    "service_endpoints": """
        Resources
        | where type =~ 'microsoft.network/virtualnetworks'
        | mv-expand subnet = properties.subnets
        | where array_length(subnet.properties.serviceEndpoints) > 0
        | project vnetName=name, subnetName=subnet.name,
    ...
    "data_share_accounts": """
        Resources
        | where type =~ 'microsoft.datashare/accounts'
        | project name, resourceGroup, location,
                  subscriptionId
    """,
    "stream_analytics": """
    ...
    "defender_easm": """
        Resources
        | where type =~ 'microsoft.easm/workspaces'
        | project name, resourceGroup, location,
                  provisioningState=properties.provisioningState,
                  subscriptionId
    """,
    ...
    "storage_accounts": """
        ...
        | where type =~ 'microsoft.storage/storageaccounts'
        | project name, resourceGroup, location,
                  storageKind=kind, sku=sku.name, accessTier=properties.accessTier,
    ...
    "service_health": """
        ...
        | take 100
    """,
    "lighthouse_registrations": """
        Resources
        | where type =~ 'microsoft.managedservices/registrationassignments'
              or type =~ 'microsoft.managedservices/registrationdefinitions'
        | project name, type, resourceGroup, location,
                  managedByTenantId=properties.managedByTenantId,
                  subscriptionId
    """,
