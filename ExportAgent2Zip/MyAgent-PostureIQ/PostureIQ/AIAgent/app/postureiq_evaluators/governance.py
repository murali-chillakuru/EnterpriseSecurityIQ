...
def evaluate_governance(
    ...
    dispatch = {
        ...
        "check_lighthouse_governance": _check_lighthouse_governance,
        "check_update_manager_compliance": _check_update_manager_compliance,
        "check_blueprint_governance": _check_blueprint_governance,
    }
...
# ---------------------------------------------------------------------------
# v55 — Azure Lighthouse Governance
# ---------------------------------------------------------------------------
def _check_lighthouse_governance(cid, ctrl, evidence, idx, thresholds=None):
    """Check Azure Lighthouse delegations for least-privilege and scope management."""
    findings = []
    delegations = [r for r in idx.get("azure-resource", [])
                   if "managedby" in (r.get("Data", {}).get("ResourceType") or "").lower()
                   or "lighthouse" in (r.get("Data", {}).get("ResourceType") or "").lower()
                   or "registrationassignment" in (r.get("Data", {}).get("ResourceType") or "").lower()]

    for item in delegations:
        d = item.get("Data", {})
        name = d.get("Name", "unknown")
        r = _res(item, d.get("ResourceType", "Microsoft.ManagedServices/registrationAssignments"))

        # Check if delegation grants Owner or Contributor (overly broad)
        role = d.get("RoleDefinitionId") or d.get("Role", "")
        if "owner" in str(role).lower():
            findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                              f"Lighthouse delegation '{name}' grants Owner role — overly permissive.",
                              recommendation="Restrict Lighthouse delegations to least-privilege roles (Reader, specific Contributor).", **r))

    if not findings:
        if delegations:
            findings.append(_f(cid, ctrl, Status.COMPLIANT,
                              f"Lighthouse governance adequate ({len(delegations)} delegation(s))."))
        else:
            findings.append(_f(cid, ctrl, Status.COMPLIANT, "No Lighthouse delegations to evaluate."))
    return findings


# ---------------------------------------------------------------------------
# v55 — Update Manager Patch Compliance
# ---------------------------------------------------------------------------
def _check_update_manager_compliance(cid, ctrl, evidence, idx, thresholds=None):
    """Check Azure Update Manager patch assessment and compliance state."""
    findings = []
    vms = idx.get("azure-vm-config", [])
    update_assessments = [r for r in idx.get("azure-resource", [])
                          if "patchassessmentresult" in (r.get("Data", {}).get("ResourceType") or "").lower()
                          or "updateassessment" in (r.get("Data", {}).get("ResourceType") or "").lower()]

    # Check VMs for missing patches via their properties
    missing_critical = 0
    assessed_vms = 0
    for vm in vms:
        d = vm.get("Data", {})
        patches = d.get("PatchStatus") or d.get("AvailablePatchSummary", {})
        if isinstance(patches, dict):
            crit = patches.get("CriticalAndSecurityPatchCount", 0) or patches.get("CriticalCount", 0)
            if crit:
    ...
    if missing_critical > 0:
        findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                          f"{missing_critical} critical/security patches missing across {assessed_vms} VM(s).",
                          recommendation="Apply missing critical patches via Azure Update Manager or schedule maintenance windows."))

    if not vms and not update_assessments:
        findings.append(_f(cid, ctrl, Status.COMPLIANT, "No VMs or update assessments to evaluate."))

    if not findings:
        findings.append(_f(cid, ctrl, Status.COMPLIANT,
                          f"Update Manager compliance adequate ({len(vms)} VM(s) assessed)."))
    return findings


# ---------------------------------------------------------------------------
# v55 — Blueprint / Deployment Stack Governance
# ---------------------------------------------------------------------------
def _check_blueprint_governance(cid, ctrl, evidence, idx, thresholds=None):
    """Check for Azure Blueprint or Deployment Stack usage for governed deployments."""
    findings = []
    blueprints = [r for r in idx.get("azure-resource", [])
                  if "blueprint" in (r.get("Data", {}).get("ResourceType") or "").lower()]
    policy_assignments = idx.get("azure-policy-assignment", [])

    # Blueprints or deployment stacks
    if blueprints:
        findings.append(_f(cid, ctrl, Status.COMPLIANT,
                          f"{len(blueprints)} Azure Blueprint/Deployment Stack resource(s) configured."))
    else:
        # No blueprints — check if policies and locks provide equivalent governance
        locks = idx.get("azure-resource-lock", [])
        initiatives = [p for p in policy_assignments if p.get("Data", {}).get("IsPolicySet")]
        if initiatives and locks:
            findings.append(_f(cid, ctrl, Status.COMPLIANT,
                              "No Blueprints detected, but policy initiatives and resource locks provide governance."))
        else:
            findings.append(_f(cid, ctrl, Status.INFO,
                              "No Azure Blueprints or Deployment Stacks. Consider using them for governed, repeatable deployments.",
                              recommendation="Use Azure Blueprints or Deployment Stacks for auditable, governed environment provisioning."))
