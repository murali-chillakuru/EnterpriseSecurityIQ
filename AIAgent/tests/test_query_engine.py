...
from app.cloud_explorer.orchestrator import COMPOSITE_NAMES
...
class TestARGTemplates(unittest.TestCase):
    ...
    def test_expected_templates_exist(self):
        expected = [
            ...
            "nsg_open_rules", "all_nsgs", "spot_vms",
            "application_insights", "compute_galleries",
            "dns_private_resolvers", "keyvaults", "aks_clusters",
...
class TestNLMappings(unittest.TestCase):
    ...
    def test_arg_mappings_have_valid_templates(self):
        valid_names = set(ARG_TEMPLATES) | set(COMPOSITE_NAMES)
        for keywords, template_name in _NL_ARG_MAP:
            self.assertIn(template_name, valid_names,
    ...
    def test_entra_mappings_have_valid_types(self):
        valid_types = {
            "disabled_users", "guest_users", "admin_users", "directory_roles",
            "conditional_access", "apps", "service_principals", "groups", "users",
            "stale_users", "risky_users", "named_locations", "auth_methods",
    ...
    def test_keyword_matching_vm(self):
        ...
        for keywords, template_name in _NL_ARG_MAP:
            if any(kw in q.lower() for kw in keywords):
                ...
                break
        self.assertEqual(matched, "all_vms")
    ...
    def test_keyword_matching_global_admin(self):
        ...
        for keywords, query_type in _NL_ENTRA_MAP:
            if any(kw in q.lower() for kw in keywords):
                matched = query_type
                break
        self.assertEqual(matched, "admin_users")
    ...
    def test_keyword_matching_all_nsgs(self):
        q = "show all NSGs"
        matched = None
        for keywords, template_name in _NL_ARG_MAP:
            if any(kw in q.lower() for kw in keywords):
                matched = template_name
                break
        self.assertEqual(matched, "all_nsgs")

    def test_keyword_matching_spot_vms(self):
        q = "show Azure Spot VMs"
        matched = None
        for keywords, template_name in _NL_ARG_MAP:
            if any(kw in q.lower() for kw in keywords):
                matched = template_name
                break
        self.assertEqual(matched, "spot_vms")

    def test_keyword_matching_compute_gallery(self):
        q = "list Azure Compute Galleries"
        matched = None
        for keywords, template_name in _NL_ARG_MAP:
            if any(kw in q.lower() for kw in keywords):
                matched = template_name
                break
        self.assertEqual(matched, "compute_galleries")

    def test_keyword_matching_application_insights(self):
        q = "show Azure Application Insights resources"
        matched = None
        for keywords, template_name in _NL_ARG_MAP:
            if any(kw in q.lower() for kw in keywords):
                matched = template_name
                break
        self.assertEqual(matched, "application_insights")

    def test_keyword_matching_private_resolver(self):
        q = "list Azure DNS Private Resolver instances"
        matched = None
        for keywords, template_name in _NL_ARG_MAP:
            if any(kw in q.lower() for kw in keywords):
                matched = template_name
                break
        self.assertEqual(matched, "dns_private_resolvers")
...
class TestAgentToolRegistration(unittest.TestCase):
    ...
    def test_tools_list_has_search_tenant(self):
        ...
        self.assertIn("run_postureiq_assessment", tool_names)
