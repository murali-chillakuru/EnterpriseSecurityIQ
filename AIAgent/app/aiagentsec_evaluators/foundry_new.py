...
from .finding import _as_finding, _unable_to_assess
...
def analyze_foundry_prompt_shields(evidence_index: dict[str, list[dict]]) -> list[dict]:
    ...
    if not evidence_index.get("azure-openai-content-filter"):
        return [_unable_to_assess("foundry_prompt_shields",
...
def analyze_foundry_model_catalog(evidence_index: dict[str, list[dict]]) -> list[dict]:
    ...
    if not evidence_index.get("azure-openai-deployment"):
        return [_unable_to_assess("foundry_model_catalog",
...
def analyze_foundry_data_exfiltration(evidence_index: dict[str, list[dict]]) -> list[dict]:
    ...
    if not evidence_index.get("azure-ai-workspace"):
        return [_unable_to_assess("foundry_data_exfiltration",
                                  ["azure-ai-workspace"])]
    findings: list[dict] = []
...
def analyze_foundry_agent_identity(evidence_index: dict[str, list[dict]]) -> list[dict]:
    ...
    if not evidence_index.get("foundry-project") and not evidence_index.get("foundry-agent-application"):
        return [_unable_to_assess("foundry_agent_identity",
                                  ["foundry-project", "foundry-agent-application"])]
    findings: list[dict] = []
...
def analyze_foundry_agent_application(evidence_index: dict[str, list[dict]]) -> list[dict]:
    ...
    if not evidence_index.get("foundry-agent-application") and not evidence_index.get("foundry-agent-deployment"):
        return [_unable_to_assess("foundry_agent_application",
...
def analyze_foundry_mcp_tools(evidence_index: dict[str, list[dict]]) -> list[dict]:
    ...
    if not evidence_index.get("azure-ai-connection"):
        return [_unable_to_assess("foundry_mcp_tools",
                                  ["azure-ai-connection"])]
    findings: list[dict] = []
...
def analyze_foundry_tool_security(evidence_index: dict[str, list[dict]]) -> list[dict]:
    ...
    if not evidence_index.get("azure-ai-connection"):
        return [_unable_to_assess("foundry_tool_security",
                                  ["azure-ai-connection"])]
    findings: list[dict] = []
...
def analyze_foundry_guardrails(evidence_index: dict[str, list[dict]]) -> list[dict]:
    ...
    if not evidence_index.get("foundry-agent-application"):
        return [_unable_to_assess("foundry_guardrails",
...
def analyze_foundry_hosted_agents(evidence_index: dict[str, list[dict]]) -> list[dict]:
    ...
    if not evidence_index.get("foundry-capability-host"):
        return [_unable_to_assess("foundry_hosted_agents",
...
def analyze_foundry_data_resources(evidence_index: dict[str, list[dict]]) -> list[dict]:
    ...
    if not evidence_index.get("azure-ai-connection") and not evidence_index.get("azure-ai-service"):
        return [_unable_to_assess("foundry_data_resources",
                                  ["azure-ai-connection", "azure-ai-service"])]
    findings: list[dict] = []
...
def analyze_foundry_observability(evidence_index: dict[str, list[dict]]) -> list[dict]:
    ...
    if not evidence_index.get("azure-ai-workspace-diagnostics") and not evidence_index.get("foundry-project"):
        return [_unable_to_assess("foundry_observability",
                                  ["azure-ai-workspace-diagnostics", "foundry-project"])]
    findings: list[dict] = []
...
def analyze_foundry_lifecycle(evidence_index: dict[str, list[dict]]) -> list[dict]:
    ...
    if not evidence_index.get("foundry-project") and not evidence_index.get("foundry-agent-application"):
        return [_unable_to_assess("foundry_lifecycle",
                                  ["foundry-project", "foundry-agent-application"])]
    findings: list[dict] = []
