...
# ── Early-skip: map collectors → required ARM resource types ─────────
# If NONE of a collector's required types exist in the inventory, the
# collector is skipped (no API calls made).  Collectors not listed here
# always run (they cover cross-resource or subscription-level data).
_COLLECTOR_REQUIRED_TYPES: dict[str, list[str]] = {
    "collect_azure_storage": ["Microsoft.Storage/storageAccounts"],
    "collect_azure_compute": ["Microsoft.Compute/virtualMachines", "Microsoft.Web/sites",
                              "Microsoft.Sql/servers", "Microsoft.ContainerService/managedClusters"],
    "collect_azure_network": ["Microsoft.Network/networkSecurityGroups", "Microsoft.Network/virtualNetworks"],
    "collect_azure_network_expanded": ["Microsoft.Network/azureFirewalls", "Microsoft.Network/routeTables"],
    "collect_azure_containers": ["Microsoft.ContainerRegistry/registries", "Microsoft.App/containerApps"],
    "collect_azure_databases": ["Microsoft.DocumentDB/databaseAccounts",
                                "Microsoft.DBforPostgreSQL/flexibleServers",
                                "Microsoft.DBforMySQL/flexibleServers"],
    "collect_azure_webapp_detailed": ["Microsoft.Web/sites"],
    "collect_azure_functions": ["Microsoft.Web/sites"],
    "collect_azure_api_management": ["Microsoft.ApiManagement/service"],
    "collect_azure_app_gateway": ["Microsoft.Network/applicationGateways"],
    "collect_azure_disks_snapshots": ["Microsoft.Compute/disks", "Microsoft.Compute/snapshots"],
    "collect_azure_messaging": ["Microsoft.ServiceBus/namespaces", "Microsoft.EventHub/namespaces"],
    "collect_azure_dns": ["Microsoft.Network/dnsZones", "Microsoft.Network/privateDnsZones"],
    "collect_azure_frontdoor_cdn": ["Microsoft.Network/frontDoors", "Microsoft.Cdn/profiles"],
    "collect_azure_sql_detailed": ["Microsoft.Sql/servers"],
    "collect_azure_redis_iot_logic": ["Microsoft.Cache/redis", "Microsoft.Devices/IotHubs",
                                      "Microsoft.Logic/workflows"],
    "collect_azure_backup_dr": ["Microsoft.RecoveryServices/vaults"],
    "collect_azure_data_analytics": ["Microsoft.Synapse/workspaces", "Microsoft.DataFactory/factories",
                                     "Microsoft.Databricks/workspaces"],
    "collect_azure_ml_cognitive": ["Microsoft.MachineLearningServices/workspaces",
                                   "Microsoft.CognitiveServices/accounts"],
    "collect_azure_batch_aci": ["Microsoft.Batch/batchAccounts",
                                "Microsoft.ContainerInstance/containerGroups"],
    "collect_azure_rdbms_detailed": ["Microsoft.DBforPostgreSQL/flexibleServers",
                                     "Microsoft.DBforMySQL/flexibleServers"],
    "collect_azure_ai_services": ["Microsoft.CognitiveServices/accounts",
                                  "Microsoft.MachineLearningServices/workspaces"],
    "collect_azure_arc": ["Microsoft.HybridCompute/machines",
                          "Microsoft.Kubernetes/connectedClusters"],
    "collect_azure_acr_data_plane": ["Microsoft.ContainerRegistry/registries"],
    "collect_azure_apim_data_plane": ["Microsoft.ApiManagement/service"],
    "collect_azure_cosmosdb_data_plane": ["Microsoft.DocumentDB/databaseAccounts"],
    "collect_azure_storage_data_plane": ["Microsoft.Storage/storageAccounts"],
    "collect_azure_aks_in_cluster": ["Microsoft.ContainerService/managedClusters"],
    "collect_azure_sentinel": ["Microsoft.OperationalInsights/workspaces"],
    "collect_foundry_config": ["Microsoft.MachineLearningServices/workspaces"],
    "collect_azure_ai_content_safety": ["Microsoft.CognitiveServices/accounts"],
}
...
async def run_postureiq_assessment(
    ...
    effective_output_dir = str(pathlib.Path(effective_output_dir) / ts)
    ...
    if config.collectors.azure_enabled:
        # Early-skip: remove collectors whose resource types don't exist
        present_types = set(inventory.types()) if inventory.loaded else set()
        skipped_collectors: list[str] = []
        azure_fns = []
        for fn in AZURE_COLLECTORS:
            if fn.__name__ in done_set:
                continue
            required = _COLLECTOR_REQUIRED_TYPES.get(fn.__name__)
            if required and present_types and not any(rt in present_types for rt in required):
                skipped_collectors.append(fn.__name__)
                continue
            azure_fns.append(fn)
        if skipped_collectors:
            log.info("Early-skip: %d collectors skipped (resource types absent): %s",
                     len(skipped_collectors), ", ".join(skipped_collectors))
    ...
    if access_denied_collectors:
        log.warning("Access Denied for %d collectors (partial data): %s",
                    len(access_denied_collectors),
                    ", ".join(c["collector"] for c in access_denied_collectors))
        # Update timing entries — mark access-denied collectors as partial
        ad_names = {c["collector"] for c in access_denied_collectors}
        for t in collector_timings:
            if t["name"] in ad_names and t["status"] == "success":
                t["status"] = "partial"
    ...
    ai_fixes_max = config.collectors.ai_fixes_max
    log.info("Phase 2.4: Generating AI fix recommendations (max_findings=%d)...", ai_fixes_max)
    try:
        ai_fixes = await generate_ai_fixes_batch(results["findings"], max_findings=ai_fixes_max)
