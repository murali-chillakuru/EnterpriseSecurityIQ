...
def evaluate_data_protection(
    ...
    dispatch = {
        ...
        "check_iot_hub_security": _check_iot_hub_security,
        "check_data_explorer_security": _check_data_explorer_security,
        "check_logic_app_data_protection": _check_logic_app_data_protection,
        "check_netapp_security": _check_netapp_security,
    }
...
# ---------------------------------------------------------------------------
# v55 — IoT Hub Security
# ---------------------------------------------------------------------------
def _check_iot_hub_security(cid, ctrl, evidence, idx):
    """Check IoT Hub TLS, public access, and Defender for IoT enablement."""
    findings = []
    hubs = [r for r in idx.get("azure-resource", [])
            if "iothub" in (r.get("Data", {}).get("ResourceType") or "").lower()]

    for item in hubs:
        d = item.get("Data", {})
        name = d.get("Name", "unknown")
        r = _res(item, "Microsoft.Devices/IotHubs")

        # Check minimum TLS version
        tls = d.get("MinTlsVersion") or d.get("MinimumTlsVersion", "")
        if tls and _tls_below(str(tls)):
            findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                              f"IoT Hub '{name}' uses TLS {tls} (below 1.2).",
                              recommendation="Set IoT Hub minimum TLS version to 1.2.", **r))

        # Check public network access
        pub = d.get("PublicNetworkAccess", "Enabled")
        if str(pub).lower() == "enabled":
            findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                              f"IoT Hub '{name}' has public network access enabled.",
                              recommendation="Disable public network access and use private endpoints for IoT Hub.", **r))

    if not findings:
        findings.append(_f(cid, ctrl, Status.COMPLIANT,
                          f"IoT Hub security adequate ({len(hubs)} hub(s))."))
    return findings


# ---------------------------------------------------------------------------
# v55 — Data Explorer / Kusto Security
# ---------------------------------------------------------------------------
def _check_data_explorer_security(cid, ctrl, evidence, idx):
    """Check Data Explorer clusters for encryption, network isolation, and managed identity."""
    findings = []
    clusters = [r for r in idx.get("azure-resource", [])
                if "kusto" in (r.get("Data", {}).get("ResourceType") or "").lower()
                or "dataexplorer" in (r.get("Data", {}).get("ResourceType") or "").lower()]

    for item in clusters:
        d = item.get("Data", {})
        name = d.get("Name", "unknown")
        r = _res(item, "Microsoft.Kusto/clusters")

        # Disk encryption
        if d.get("EnableDiskEncryption") is False:
            findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                              f"Data Explorer cluster '{name}' has disk encryption disabled.",
                              recommendation="Enable disk encryption on the Data Explorer cluster.", **r))

        # Double encryption
        if d.get("EnableDoubleEncryption") is False:
            findings.append(_f(cid, ctrl, Status.INFO,
                              f"Data Explorer cluster '{name}' does not use double encryption.",
                              recommendation="Consider enabling double encryption for defense-in-depth.", **r))

        # Public network access
        pub = d.get("PublicNetworkAccess", "")
        if str(pub).lower() == "enabled":
            findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                              f"Data Explorer cluster '{name}' has public network access enabled.",
                              recommendation="Restrict public access and configure private endpoints.", **r))

    if not findings:
        findings.append(_f(cid, ctrl, Status.COMPLIANT,
                          f"Data Explorer security adequate ({len(clusters)} cluster(s))."))
    return findings


# ---------------------------------------------------------------------------
# v55 — Logic App Data Protection
# ---------------------------------------------------------------------------
def _check_logic_app_data_protection(cid, ctrl, evidence, idx):
    """Check Logic Apps for managed identity usage, HTTPS triggers, and access control."""
    findings = []
    apps = [r for r in idx.get("azure-resource", [])
            if "workflow" in (r.get("Data", {}).get("ResourceType") or "").lower()
            or "logicapp" in (r.get("Data", {}).get("ResourceType") or "").lower()]

    for item in apps:
        d = item.get("Data", {})
        name = d.get("Name", "unknown")
        r = _res(item, "Microsoft.Logic/workflows")

        # Managed identity
        identity = d.get("Identity") or d.get("ManagedIdentityType")
        if not identity:
            findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                              f"Logic App '{name}' has no managed identity configured.",
                              recommendation="Enable managed identity for Logic App to avoid stored credentials.", **r))

        # Access control / IP restrictions
        ac = d.get("AccessControl") or d.get("IpRestrictions")
        if not ac:
            findings.append(_f(cid, ctrl, Status.INFO,
                              f"Logic App '{name}' has no explicit access control configuration.",
                              recommendation="Configure trigger and action access control to restrict invocations.", **r))

    if not findings:
        findings.append(_f(cid, ctrl, Status.COMPLIANT,
                          f"Logic App data protection adequate ({len(apps)} workflow(s))."))
    return findings


# ---------------------------------------------------------------------------
# v55 — NetApp Files Security
# ---------------------------------------------------------------------------
def _check_netapp_security(cid, ctrl, evidence, idx):
    """Check NetApp Files accounts/volumes for encryption and network restrictions."""
    findings = []
    accounts = [r for r in idx.get("azure-resource", [])
                if "netapp" in (r.get("Data", {}).get("ResourceType") or "").lower()]

    for item in accounts:
        d = item.get("Data", {})
        name = d.get("Name", "unknown")
        rtype = d.get("ResourceType", "Microsoft.NetApp/netAppAccounts")
        r = _res(item, rtype)

        # Encryption type
        enc = d.get("EncryptionType") or d.get("Encryption", "")
        if enc and "microsoft" not in str(enc).lower() and "customer" not in str(enc).lower():
            findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                              f"NetApp resource '{name}' has unclear encryption setting.",
                              recommendation="Verify encryption at rest is enabled with Microsoft or customer-managed keys.", **r))

        # Volume protocol check — NFS v3 has no encryption in transit
        protocol = d.get("ProtocolTypes") or d.get("Protocol", "")
        if "nfsv3" in str(protocol).lower():
            findings.append(_f(cid, ctrl, Status.NON_COMPLIANT,
                              f"NetApp volume '{name}' uses NFSv3 which does not support encryption in transit.",
                              recommendation="Use NFSv4.1 with Kerberos for encryption in transit.", **r))

    if not findings:
        findings.append(_f(cid, ctrl, Status.COMPLIANT,
                          f"NetApp Files security adequate ({len(accounts)} resource(s))."))
    return findings
