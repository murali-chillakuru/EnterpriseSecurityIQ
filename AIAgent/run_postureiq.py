"""One-shot full-tenant assessment across all 11 frameworks."""
import asyncio
import pathlib
...
FRAMEWORKS = [
    "FedRAMP", "PCI-DSS", "NIST-800-53", "MCSB", "CIS",
    "ISO-27001", "SOC2", "NIST-CSF", "HIPAA", "GDPR", "CSA-CCM",
]


async def main():
    out = str(_REPO_ROOT / "output")

    config = AssessmentConfig.from_env()
    config.frameworks = FRAMEWORKS
    config.output_formats = ["json", "html", "md"]
    config.collectors.entra_enabled = True   # Full tenant (Azure + Entra)
    ...
    print("Scope: Full tenant (Azure + Entra)")
    print(f"Output: {out}")
    print()

    result = await run_postureiq_assessment(creds, config=config, output_dir=out)
    ...
    print("=" * 60)
    for fw, d in s.get("FrameworkSummaries", {}).items():
        sc = d.get("ComplianceScore", 0)
        c = d.get("Compliant", 0)
        tot = d.get("TotalControls", 0)
        print(f"  {fw}: {sc:.1f}% ({c}/{tot} compliant)")
